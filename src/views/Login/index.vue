...
const rules = {
    ...
    password:[
        ...
        {min:6,max:14,message:'密码应该为6-14位',trigger:'blur'}
    ],
    agree:[
        // 自定义表单校验，使用validator函数实现
        {
            validator: (rule, val, callback) => {
                if(val){
                    callback()
                }else{
                    callback(new Error('还未勾选协议'))
                }
            }
        }
    ]
...
const onLogin = () => {
    ...
    //调用validate方法进行全局校验，只有全部校验都通过时vaild才会为true，验证通过后才能进行后续的登录操作
    fromRef.value.validate(async (vaild) => {
        if(vaild){
            //
